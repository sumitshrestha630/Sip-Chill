// Stylized SVG bottle placeholder. Replace with real photography in production.
const Bottle = ({ color = "burgundy", label = "white", w = 80 }) => {
  ...
  // narrow neck for wine/spirit, wider for beer
  ...
  return (
    <svg width={w} height={h} viewBox="0 0 100 250" style={{display:"block", filter:"drop-shadow(0 18px 22px rgba(0,0,0,0.45))"}}>
      ...
      {/* Neck */}
      <rect x={50 - neckW * 50 / 2} y={isSparkling ? 32 : 24} width={neckW * 50} height={shoulderY * 250 - (isSparkling ? 32 : 24)} fill={`url(#bg-${color}-${label})`}/>
      {/* Body */}
      <path d={`M 18 ${shoulderY * 250 + 12}
               C 18 ${shoulderY * 250}, ${50 - neckW * 50 / 2} ${shoulderY * 250}, ${50 - neckW * 50 / 2} ${shoulderY * 250 - 4}
               L ${50 + neckW * 50 / 2} ${shoulderY * 250 - 4}
               C ${50 + neckW * 50 / 2} ${shoulderY * 250}, 82 ${shoulderY * 250}, 82 ${shoulderY * 250 + 12}
               L 82 238
               Q 82 246, 74 246
               L 26 246
               Q 18 246, 18 238 Z`} fill={`url(#bg-${color}-${label})`}/>
      {/* Highlight */}
      <rect x="28" y={shoulderY * 250 + 18} width="14" height="200" fill={`url(#hi-${color}-${label})`} rx="6" opacity="0.5"/>
      {/* Label */}
