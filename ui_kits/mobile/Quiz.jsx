// Wine & Spirits Finder — five-question conversational quiz, then a recommendation.
  ...
  <div className="m-app">
    <div className="m-quiz">
      ...
      <div style={{display: "flex", flexDirection: "column", gap: 14, alignItems: "flex-start", flex: 1, justifyContent: "flex-end"}}>
        ...
        <button className="next" style={{background: "var(--amber)", color: "var(--ink)", padding: "16px 28px", borderRadius: 999, fontSize: 15, fontWeight: 600, border: "none", display: "inline-flex", alignItems: "center", gap: 8, cursor: "pointer"}}
                onClick={() => goTo("quiz-step", 0)}>
...
const QuizStep = ({ step, answers, setAnswer, goTo }) => {
  ...
  return (
    <div className="m-app">
      <div className="m-quiz">
        ...
        <h2 className="m-quiz-q" dangerouslySetInnerHTML={{ __html: Q.q.replace(Q.em, `<em>${Q.em}</em>`) }}/>
        <div className="m-quiz-options">
          {Q.opts.map(opt => (
            <button key={opt.v} className={`m-quiz-opt ${current === opt.v ? "selected" : ""}`} onClick={() => setAnswer(step, opt.v)}>
              ...
              {current === opt.v && <span className="check"><Icon name="plus" size={12} style={{transform: "rotate(45deg)"}}/></span>}
            </button>
          ))}
        </div>
...
const QuizResult = ({ answers, goTo }) => {
  // Pick a product based loosely on answers
              ...
              {alt.map(p => (
                <article key={p.id} className="m-prod">
                  <div className="m-prod-img">
                    <Bottle color={p.color} label={p.label} w={60}/>
                  </div>
